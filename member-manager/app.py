    ...
    try:
        ...
        if table_name in ["human_members", "virtual_members"] and "yml_file_uri" in data:
            ...
            try:
                ...
                return (
                    jsonify({"success": False, "error": f"UPSERT処理中にエラーが発生: {str(e)}"}),
                    500,
                )
...
def get_database_tables():
    ...
    try:
        ...
        for table in tables:
            count = db_manager.get_table_count(table)
            table_info.append({"name": table, "record_count": count})
...
def create_member():
    ...
    try:
        ...
        if not storage_client.validate_storage_path(storage_path):
            return jsonify({"success": False, "message": "無効なストレージパスです"}), 400
        ...
        db_data = {"member_name": member_name, "yml_file_uri": storage_path}
...
def get_storage_info():
    ...
    try:
        ...
        return jsonify({"success": True, "data": storage_info})
