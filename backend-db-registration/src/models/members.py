...
class HumanMemberProfile(Base):
    ...
    member_uuid = Column(UUID(as_uuid=True), ForeignKey('human_members.member_uuid'), nullable=False, unique=True)
    bio = Column(Text)
    created_at = Column(DateTime, default=lambda: datetime.datetime.now(datetime.UTC))
    updated_at = Column(DateTime, default=lambda: datetime.datetime.now(datetime.UTC), onupdate=lambda: datetime.datetime.now(datetime.UTC))
...
class VirtualMemberProfile(Base):
    ...
    member_uuid = Column(UUID(as_uuid=True), ForeignKey('virtual_members.member_uuid'), nullable=False, unique=True)
