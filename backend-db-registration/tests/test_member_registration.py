...
def db_session():
    ...
    with engine.connect() as conn:
        ...
        truncate_sql = text(
            "TRUNCATE TABLE virtual_member_profiles, virtual_members, human_members RESTART IDENTITY CASCADE"
        )
...
def test_yaml_files(tmp_path):
    ...
    # 人間メンバー用のYAMLファイル
    human_yaml = {"name": "テスト太郎", "bio": "テスト用の人間メンバーです"}
    ...
    return {"human": str(human_yaml_path), "virtual": str(virtual_yaml_path)}
...
def test_register_human_member_invalid_yaml(db_session, tmp_path, mock_storage_client):
    ...
    # 無効なYAMLファイルを作成
    invalid_yaml = {"invalid_field": "テスト太郎"}
...
def test_human_member_missing_name_validation(db_session, mock_storage_with_real_files):
    ...
    assert "人間メンバーYAMLに必須フィールドが不足しています: name" in str(
        exc_info.value
    ) or "Required fields missing in human member YAML: name" in str(exc_info.value)
...
def test_virtual_member_missing_name_validation(db_session, mock_storage_with_real_files):
    ...
    # 日本語エラーメッセージの検証
    assert "仮想メンバーYAMLに必須フィールドが不足しています: name" in str(
        exc_info.value
    ) or "Required fields missing in virtual member YAML: name" in str(exc_info.value)
    assert "name" in exc_info.value.missing_fields
...
def test_yml_file_uri_based_upsert_human_member(db_session, monkeypatch):
    ...
    try:
        ...
        profile = (
            session.query(HumanMemberProfile)
            .filter(HumanMemberProfile.member_uuid == member1.member_uuid)
            .first()
        )
    ...
    try:
        profile = (
            session.query(HumanMemberProfile)
            .filter(HumanMemberProfile.member_uuid == member2.member_uuid)
            .first()
        )
        assert profile is not None
...
def test_yml_file_uri_based_upsert_virtual_member(db_session, monkeypatch):
    ...
    try:
        ...
        profile = (
            session.query(VirtualMemberProfile)
            .filter(VirtualMemberProfile.member_uuid == member1.member_uuid)
            .first()
        )
        assert profile is not None
    ...
    try:
        profile = (
            session.query(VirtualMemberProfile)
            .filter(VirtualMemberProfile.member_uuid == member2.member_uuid)
            .first()
        )
        assert profile is not None
...
def test_profile_information_storage(db_session, monkeypatch):
    ...
    try:
        ...
        human_profile = (
            session.query(HumanMemberProfile)
            .filter(HumanMemberProfile.member_uuid == human_member.member_uuid)
            .first()
        )
    ...
    try:
        ...
        virtual_profile = (
            session.query(VirtualMemberProfile)
            .filter(VirtualMemberProfile.member_uuid == virtual_member.member_uuid)
            .first()
        )
        assert virtual_profile is not None
        assert virtual_profile.llm_model == "claude-3-opus"
        assert (
            virtual_profile.custom_prompt
            == "私は詳細なプロンプトを持つAIアシスタントです。質問があれば何でも聞いてください。"
        )
